...
% Extract states
alt22 = output_temp.result.solution.phase(2).state(:,1).';
lon22 = output_temp.result.solution.phase(2).state(:,2).';
lat22 = output_temp.result.solution.phase(2).state(:,3).';
v22 = output_temp.result.solution.phase(2).state(:,4).'; 
gamma22 = output_temp.result.solution.phase(2).state(:,5).'; 
zeta22 = output_temp.result.solution.phase(2).state(:,6).';
Alpha22 = output_temp.result.solution.phase(2).state(:,7).';
eta22 = output_temp.result.solution.phase(2).state(:,8).';
mFuel22 = output_temp.result.solution.phase(2).state(:,9).'; 
time22 = output_temp.result.solution.phase(2).time.';
throttle22 = output_temp.result.solution.phase(2).state(:,10).';
...
alt21 = output.result.solution.phase(1).state(:,1).';
alt22 = output.result.solution.phase(2).state(:,1).';
lon21 = output.result.solution.phase(1).state(:,2).';
lon22 = output.result.solution.phase(2).state(:,2).';
lat21 = output.result.solution.phase(1).state(:,3).';
lat22 = output.result.solution.phase(2).state(:,3).';
v21 = output.result.solution.phase(1).state(:,4).'; 
v22 = output.result.solution.phase(2).state(:,4).'; 
gamma21 = output.result.solution.phase(1).state(:,5).'; 
gamma22 = output.result.solution.phase(2).state(:,5).'; 
zeta21 = output.result.solution.phase(1).state(:,6).';
zeta22 = output.result.solution.phase(2).state(:,6).';
Alpha21 = output.result.solution.phase(1).state(:,7).';
Alpha22 = output.result.solution.phase(2).state(:,7).';
eta21 = output.result.solution.phase(1).state(:,8).';
eta22 = output.result.solution.phase(2).state(:,8).';
mFuel21 = output.result.solution.phase(1).state(:,9).'; 
mFuel22 = output.result.solution.phase(2).state(:,9).'; 

throttle22 = output.result.solution.phase(2).state(:,10).';

aoadot21  = output.result.solution.phase(1).control(:,1).'; 
etadot21  = output.result.solution.phase(1).control(:,2).'; 

aoadot22  = output.result.solution.phase(2).control(:,1).'; 
etadot22  = output.result.solution.phase(2).control(:,2).'; 

time21 = output.result.solution.phase(1).time.';
time22 = output.result.solution.phase(2).time.';
...
[~,~,~,~,~,~, q1, M1, Fd1, rho1,L1,Fueldt1,T1,Isp1,q11,flapdeflection,heating_rate] = VehicleModelCombined(gamma21, alt21, v21,auxdata,zeta21,lat21,lon21,Alpha21,eta21,1, mFuel21,mFuel21(1),mFuel21(end), 1);
[~,~,~,~,~,~, q2, M2, Fd2, rho2,L2,Fueldt2,T2,Isp2,q12,flapdeflection2,heating_rate2] = VehicleModelCombined(gamma22, alt22, v22,auxdata,zeta22,lat22,lon22,Alpha22,eta22,throttle22, mFuel22,0,0, 0);

throttle22(M2<5.1) = 0; % remove nonsense throttle points
...
subplot(7,2,4)
plot(time21, M1,'Color','k')
title('Mach no')

subplot(7,2,5)
plot(time21, q1/1000,'Color','k')
...
subplot(7,2,9)
plot(time21, flapdeflection,'Color','k')
...
hold on
xlim([time22(1) time22(end)]);
plot(time22, M2,'Color','k')
title('Mach no')

subplot(7,2,4)
hold on
xlim([time22(1) time22(end)]);
plot(time22, q2/1000,'Color','k')
title('Dynamic Pressure (kpa)')
...
hold on
xlim([time22(1) time22(end)]);
plot(time22, flapdeflection2,'Color','k')
title('Flap Deflection (deg)')

subplot(7,2,9)
hold on
xlim([time22(1) time22(end)]);
plot(time22, T2/1000,'Color','k')
title('Thrust (kN)')

subplot(7,2,10)
hold on
xlim([time22(1) time22(end)]);
plot(time22, Isp2,'Color','k')
...
plot(time21,([diff(output.result.solution.phase(1).state(:,i))./diff(output.result.solution.phase(1).time); 0] - phaseout_test(1).dynamics(:,i))./output.result.solution.phase(1).state(:,i),'--');
end
for i = 1:length(output.result.solution.phase(2).state(1,:))
    if i<= 7 % Plot different line styles when no. of colours exceeded
    plot(time22,([diff(output.result.solution.phase(2).state(:,i))./diff(output.result.solution.phase(2).time); 0] - phaseout_test(2).dynamics(:,i))./output.result.solution.phase(2).state(:,i));
    else
    plot(time22,([diff(output.result.solution.phase(2).state(:,i))./diff(output.result.solution.phase(2).time); 0] - phaseout_test(2).dynamics(:,i))./output.result.solution.phase(2).state(:,i),':');
    end
end
...
T_in1 = auxdata.interp.tempgridded(M1,rad2deg(Alpha21)).*T0;
M_in1 = auxdata.interp.M1gridded(M1, rad2deg(Alpha21));
...
mFuel21 = 0.939*(mRocket-mEngine); % Maximum fuel, structural mass fraction calculated without engine
mSpartan = 9819.11;

FirstStageSMF = (mRocket - mFuel21)/(m1(1) - mSpartan);
