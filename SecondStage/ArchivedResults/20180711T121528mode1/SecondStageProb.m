%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Launch System Flight Optimiser
...
%% Launch Point
lat0 = deg2rad(-12.418); % Equatorial Launch Australia Spaceport, 25km south of Nhulunbuy
lon0 = deg2rad(136.876);
...
aoaMin21 = -deg2rad(5);  aoaMax21 = 10*pi/180;
bankMin21 = -1*pi/180; bankMax21 =   90*pi/180;

% Stage2.mFuel = 1000

% Bounds %-----------------------------------------------------------------
Stage2.Bounds.Alt       = [20000, 55000]; % Altitude Bounds, m
...
Stage2.Bounds.time      = [100, 2000]; % Time Bounds, s
...
bounds.phase(2).finalstate.lower = [Stage2.Bounds.Alt(1), lonMin, latMin2, 2300, 0, Stage2.Bounds.zeta(1), aoaMin21, BankSep, Stage2.Bounds.mFuel(1)];
bounds.phase(2).finalstate.upper = [Stage2.Bounds.Alt(2), lonMax, latMax2, Stage2.Bounds.v(2), deg2rad(20), Stage2.Bounds.zeta(2), aoaMax21, BankSep, Stage2.mFuel];
...
elseif mode ==3 || mode == 32
        bounds.phase(2).path.lower = [49900];
    bounds.phase(2).path.upper = [50100];
end
...
Stage2.Guess.gamma      = [0.0, 0.0]; % Trajectory Angle guess, rad
Stage2.Guess.zeta       = [1.85, 1.85]; % Heading Angle Guess, rad

guess.phase(2).state(:,2)   = [lon0;lon0];
guess.phase(2).state(:,3)   = [lat0;lat0];
guess.phase(2).state(:,4)   = Stage2.Guess.v.';
guess.phase(2).state(:,5)   = Stage2.Guess.gamma.';
guess.phase(2).state(:,6)   = Stage2.Guess.zeta.';
guess.phase(2).state(:,7)   = [0*pi/180; 0*pi/180];
...
% bounds.phase(3).finalstate.lower = [altMin, lonMin, latMin2, speedMin, deg2rad(-20), aziMin, aoaMin21, bankMin21, mFuelF, throttleMin];
% bounds.phase(3).finalstate.upper = [altMax, lonMax, latMax2, speedMax, deg2rad(30), aziMax, aoaMax21, bankMax21, mFuelF, throttleMax];
...
% 
% tfMin3 = 7;            tfMax3 = 200;
altMin3 = 30000;  altMax3 = 200000;
phiMin3 = -0.5;         phiMax3 = 0.5;
vMin3 = 10;        vMax3 = 8000;
gammaMin3=deg2rad(-5);  gammaMax3 =  deg2rad(30);
zetaMin3 = deg2rad(50); zetaMax3 =  deg2rad(140);
...
bounds.phase(phase_no).initialstate.lower = [altMin3,vMin3, gammaMin3,  auxdata.Stage3.mTot, aoaMin3, phiMin3, zetaMin3];
...
[altdot21,xidot21,phidot21,gammadot21,a21,zetadot21, q21, M21, Fd21, rho21,L21,Fueldt21,T21,Isp21,q121,flapdeflection21,heating_rate21] = VehicleModelCombined(gamma21, alt21, v21,auxdata,zeta21,lat21,lon21,alpha21,eta21,ones(1,length(alt21)), mFuel21,mFuel21(1),mFuel21(end), 1, 0);
if returnflag
