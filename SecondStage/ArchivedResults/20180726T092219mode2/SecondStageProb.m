...
elseif mode == 2
    q_vars = [40000 45000 50000 55000 60000]
...
norm_error1 = [];
for num = 1:length(output_temp.result.solution.phase(2).state(1,:))
norm_error1(num,:) = (diff(output_temp.result.solution.phase(2).state(:,num))./diff(output_temp.result.solution.phase(2).time) - phaseout_test(2).dynamics(1:end-1,num))./max(abs(phaseout_test(2).dynamics(1:end-1,num)));
end
norm_error2 = [];
for num = 1:length(output_temp.result.solution.phase(3).state(1,:))
norm_error2(num,:) = (diff(output_temp.result.solution.phase(3).state(:,num))./diff(output_temp.result.solution.phase(3).time) - phaseout_test(3).dynamics(1:end-1,num))./max(abs(phaseout_test(3).dynamics(1:end-1,num)));
end

error(i) = sum(sum(abs(norm_error1)))+sum(sum(abs(norm_error2)));
...
Plotter(output,auxdata,mode,M_englist,T_englist,engine_data,MList_EngineOn,AOAList_EngineOn,mRocket,mSpartan,mFuel,h0,v0,bounds);
