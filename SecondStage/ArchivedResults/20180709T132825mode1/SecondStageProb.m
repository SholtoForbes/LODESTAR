...
% Bounds %-----------------------------------------------------------------
Stage21.Bounds.Alt       = [20000, 50000]; % Altitude Bounds, m
Stage21.Bounds.v         = [1400, 3100]; % Velocity Bounds, m/s
Stage21.Bounds.gamma     = [-0.5, deg2rad(6)]; % Trajectory Angle Bounds, rad
Stage21.Bounds.mFuel     = [0, Stage21.mFuel]; % Fuel Mass Bounds, kg
Stage21.Bounds.gammadot  = [-0.01, 0.02]; % Trajectory Angle Derivative Bounds, rad/s
Stage21.Bounds.zeta      = [-4/3*pi, 2*pi]; % Heading Angle Bounds, rad
Stage21.Bounds.control   = [-0.00003, 0.0002]; % (Control) Trajectory Angle Double-Derivative Bounds, rad/s^2
Stage21.Bounds.time      = [100, 800]; % Time Bounds, s




% Primal Bounds
bounds.phase(2).state.lower = [Stage21.Bounds.Alt(1), lonMin, latMin2, Stage21.Bounds.v(1), Stage21.Bounds.gamma(1), Stage21.Bounds.zeta(1), aoaMin21, bankMin21, Stage21.Bounds.mFuel(1)];
bounds.phase(2).state.upper = [Stage21.Bounds.Alt(2), lonMax, latMax2, Stage21.Bounds.v(2), Stage21.Bounds.gamma(2), Stage21.Bounds.zeta(2), aoaMax21, bankMax21, Stage21.Bounds.mFuel(2)];

% Initial States
bounds.phase(2).initialstate.lower = [Stage21.Bounds.Alt(1),lonMin, latMin2, Stage21.Bounds.v(1), Stage21.Bounds.gamma(1), Stage21.Bounds.zeta(1), aoaMin21, bankMin21, Stage21.mFuel] ;
bounds.phase(2).initialstate.upper = [Stage21.Bounds.Alt(2),lonMax, latMax2, Stage21.Bounds.v(2), Stage21.Bounds.gamma(2), Stage21.Bounds.zeta(2), aoaMax21, bankMax21, Stage21.mFuel];

% End States
% End bounds are set slightly differently, to encourage an optimal solution
bounds.phase(2).finalstate.lower = [34000, lonMin, latMin2, 2300, 0, Stage21.Bounds.zeta(1), aoaMin21, 0, Stage21.Bounds.mFuel(1)];
bounds.phase(2).finalstate.upper = [45000, lonMax, latMax2, Stage21.Bounds.v(2), deg2rad(20), Stage21.Bounds.zeta(2), aoaMax21, 0, Stage21.mFuel];
...
bounds.phase(2).initialtime.upper = Stage21.Bounds.time(2);
bounds.phase(2).finaltime.lower = Stage21.Bounds.time(1);
bounds.phase(2).finaltime.upper = Stage21.Bounds.time(2);
...
Stage21.Guess.v          = [Stage21.Initial.v, 2900]; % Velocity Guess, m/s
Stage21.Guess.gamma      = [0.0, deg2rad(4.3)]; % Trajectory Angle guess, rad
Stage21.Guess.zeta       = [0, 1.699]; % Heading Angle Guess, rad

guess.phase(2).state(:,2)   = [2.53;2.5368];
guess.phase(2).state(:,3)   = [-0.269;-0.10];
guess.phase(2).state(:,4)   = Stage21.Guess.v.';
guess.phase(2).state(:,5)   = Stage21.Guess.gamma.';
guess.phase(2).state(:,6)   = Stage21.Guess.zeta.';
guess.phase(2).state(:,7)   = [2*pi/180; 5*pi/180];
guess.phase(2).state(:,8)   = [deg2rad(10);deg2rad(10)];
guess.phase(2).state(:,9) 	= [Stage21.Initial.mFuel, 100];
...
% End State Bounds
bounds.phase(3).finalstate.lower = [altMin, lon0, lat0, speedMin, deg2rad(-20), aziMin, aoaMin21, bankMin21, Stage21.End.mFuel, throttleMin];
bounds.phase(3).finalstate.upper = [altMax, lon0, lat0, speedMax, deg2rad(30), aziMax, aoaMax21, bankMax21, Stage21.End.mFuel, throttleMax];
