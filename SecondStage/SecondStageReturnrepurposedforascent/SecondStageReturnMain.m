...
bounds.phase.finaltime.lower = tfMin;
bounds.phase.finaltime.upper = tfMax;
bounds.phase.initialstate.lower = [rad0, lon0, lat0, speed0, fpa0, azi0, aoaMin, mFuelMax];
bounds.phase.initialstate.upper = [rad0, lon0, lat0, speed0, fpa0, azi0, aoaMax, mFuelMax];

bounds.phase.state.lower = [radMin, lonMin, latMin, speedMin, fpaMin, aziMin, aoaMin, mFuelMin];
bounds.phase.state.upper = [radMax, lonMax, latMax, speedMax, fpaMax, aziMax, aoaMax, mFuelMax];

bounds.phase.finalstate.lower = [rad0+10000, lonMin, latMin, speedMin, deg2rad(-10), aziMin, aoaMin, mFuelMin];
bounds.phase.finalstate.upper = [rad0+10000, lonMax, latMax, speedMax, deg2rad(30), aziMax, aoaMax, mFuelMin];
...
bounds.phase.control.lower = [deg2rad(-.5)];
bounds.phase.control.upper = [deg2rad(.5)];
...
lonGuess            = [lon0; lon0];
latGuess            = [lat0; lat0];
speedGuess          = [speed0; speed0];
fpaGuess            = [fpa0; fpa0];
aziGuess            = [azi0; azi0];
aoaGuess            = [8*pi/180; 8*pi/180];
% mFuelGuess          = [mFuelMax; mFuelMin];
mFuelGuess          = [mFuelMin; mFuelMin];
guess.phase.state   = [radGuess, lonGuess, latGuess, speedGuess, fpaGuess, aziGuess, aoaGuess, mFuelGuess];
...
[f_t, f_y] = ode45(@(f_t,f_y) VehicleModelReturn_forward(f_t, f_y,auxdata,ControlInterp(t,aoa,f_t),0,1),t(1:end),forward0);
