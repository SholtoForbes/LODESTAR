function [AltF_actual, vF, Alt, v, t, mpayload, Alpha, m,AoA_init,q,gamma,D,AoA_max,zeta,phi, inc,Vec_angle,T,CL,L] = ThirdStageSim(x,k,j,u, phi0, zeta0)
...
%     end
    ...
    if t(i) <= x(end)
        Alpha(i) = interp1(0:x(end)/(length(x)-3):x(end),x(1:end-2),t(i),'pchip'); % interpolate between angle of attack points using an interior pchip spline
    elseif t(i) > x(end) && t(i) <= burntime
        Alpha(i) = -x(end-2)*(t(i) - x(end))/(burntime - x(end)) + x(end-2);
    elseif t(i) > burntime
        Alpha(i) = 0;
    end
