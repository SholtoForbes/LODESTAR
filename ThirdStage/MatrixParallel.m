...
for k = [32500:1000:39500]
    for j = [0:0.01:0.05]
        ...
        parfor i = 1:length(u)
        [AltF_temp, vF_temp, Alt_temp, v_temp, t_temp, mpayload_temp, Alpha_temp, m_temp,AoA_temp,q_temp,gamma_temp,D_temp,AoA_max] = ThirdStageSim([0 0 0 0 0],k,j,u(i), phi0, zeta0);
        
%         guess = [1700 AoA_max-0.01 AoA_max/2+AoA_max/2*(0.1-j)/0.1-0.01];
%         guess = [1600 AoA_max(i)-0.01];
% guess = [2700  deg2rad(7.5) deg2rad(10)];
guess = [2750  (deg2rad(14)+(deg2rad(6)-deg2rad(14))*j/0.05) (deg2rad(10)+((AoA_max-0.01)-deg2rad(10))*j/0.05) (deg2rad(14)+((AoA_max-0.01)-deg2rad(14))*j/0.05) (deg2rad(12)+(deg2rad(10)-deg2rad(12))*j/0.05)]
%         x0 = guess(:,i); 
        x0 = guess; 
