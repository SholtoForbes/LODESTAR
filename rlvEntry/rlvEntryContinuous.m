...
speedOfSound = spline(input.auxdata.Atmosphere(:,1),input.auxdata.Atmosphere(:,5),altitude);
mach = v./speedOfSound;
density = spline(input.auxdata.Atmosphere(:,1),input.auxdata.Atmosphere(:,4),altitude);
% interpolate coefficients
CD = input.auxdata.interp.Cd_spline(mach,rad2deg(aoa));
CL = input.auxdata.interp.Cl_spline(mach,rad2deg(aoa));
% 
% 
q        = 0.5*rho.*v.^2;
D        = q.*S.*CD./mass;
