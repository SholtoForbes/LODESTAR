...
ThirdStageData = dlmread('thirdstage.dat'); %Import Third Stage Data Raw (not used in main PS routine)
...
[alt_list.course,gamma_list.course,v_list.course,payload_array.course] = thirdstagemanipulation('thirdstage.dat'); %Import third stage data

[alt_list.fine,gamma_list.fine,v_list.fine,payload_array.fine] = thirdstagemanipulation('thirdstagefine.dat');
...
guess.states(1,:) =[interp1(Atmosphere(:,4),Atmosphere(:,1),2*50000/v0^2)-100 ,35300]/scale.V; %50kpa limited
% 
...
guess.states(1,:) = [interp1(Atmosphere(:,4),Atmosphere(:,1),2*45000/v0^2) ,35500];%45kPa limited
elseif const == 14
...
addpath('./');
addpath('./ThirdStage');
ThirdStageOptm(V(end),theta(end),v(end), phi(end),zeta(end))
