...
MList = (linspace(min(communicator(:,1)),max(communicator(:,1)),100).' * ones(100,1));
AOAList = (ones(1,100) * linspace(min(communicator(:,2)),max(communicator(:,2)),100));

M1_GridFit = gridfit(communicator(:,1),communicator(:,2),communicator(:,12),100,100);
pres_GridFit = gridfit(communicator(:,1),communicator(:,2),communicator(:,13),100,100);
temp_GridFit = gridfit(communicator(:,1),communicator(:,2),communicator(:,14),100,100);

scattered.M1gridded = griddedInterpolant(MList,AOAList,M1_GridFit,'spline');
scattered.presgridded = griddedInterpolant(MList,AOAList,pres_GridFit,'spline');
scattered.tempgridded = griddedInterpolant(MList,AOAList,temp_GridFit,'spline');
...
Isp_GridFit = gridfit(data(:,1),data(:,2),data(:,6),100,100);
M1List = linspace(min(data(:,1)),max(data(:,1)),100).' * ones(100,1);
TList = ones(100,1) * linspace(min(data(:,2)),max(data(:,2)),100);

scattered.IspGridded = griddedInterpolant(M1List,TList,Isp_GridFit,'spline');
...
% 
% guess.states(1,:) =[33000,33000]/scale.V; %50kpa limited
...
guess.states(3,:) = [deg2rad(1.3),0.03]/scale.theta;  
end 
