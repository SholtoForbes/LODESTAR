...
% 
guess.states(1,:) =[interp1(Atmosphere(:,4),Atmosphere(:,1),2*50000/v0^2)+100, interp1(Atmosphere(:,4),Atmosphere(:,1),2*50000/2300^2)+100 ,interp1(Atmosphere(:,4),Atmosphere(:,1),2*50000/2860^2)+100]/scale.V; %50kpa limited
...
guess.states(2,:) = [v0,2300, vf]/scale.v; %v for normal use
...
guess.states(3,:) = [deg2rad(1.3),deg2rad(1.3),0.03]/scale.theta;  
end 

guess.states(4,:) = [mfuelU,660, 0]/scale.m;

guess.states(5,:) = [0,0,0];

guess.controls(1,:)    = [0,0,0]; 

guess.time        = [t0, 73 ,tfGuess];
