...
global ThirdStageData_manip
Payload_temp = zeros(1,length(ThirdStageData_manip(1,1,:)));
for  i = 1: length(ThirdStageData_manip(1,1,:))
    if V(end) > 40000
        if theta(end) < 0  
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),gaussmf(theta(end),[0.1 0])*gaussmf(V(end),[10000 40000])*scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),40000, 0, v(end))];
        else
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),gaussmf(theta(end),[0.1 0])*gaussmf(V(end),[10000 40000])*scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),40000, theta(end), v(end))];
        end
    elseif v(end) < 2500
        if theta(end) < 0
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),gaussmf(theta(end),[0.1 0])*gaussmf(v(end),[1000 2000])*scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),V(end), 0, 2000)]; 
        else
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),gaussmf(theta(end),[0.1 0])*gaussmf(v(end),[1000 2000])*scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),V(end), theta(end), 2000)]; 
        end
    else
        if theta(end) < 0
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),gaussmf(theta(end),[0.1 0])*scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),V(end), 0, v(end))];  
        else
        Payload_temp(:,i) = [ThirdStageData_manip(1,1,i),ThirdStageData_manip(1,2,i),scattered(ThirdStageData_manip(:,3,i),ThirdStageData_manip(:,4,i),ThirdStageData_manip(:,5,i),ThirdStageData_manip(:,6,i),V(end), theta(end), v(end))];
        end
    end
end
...
disp('end values outside of search range for latitude or heading angle')
 ...
 ThirdStagePayloadMass = scattered(Payload_temp(:,1),Payload_temp(:,2),Payload_temp(:,3),phi(end),zeta(end));
