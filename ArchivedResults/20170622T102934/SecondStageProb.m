...
%% Set the equivalence ratio interpolation region! VERY IMPORTANT

% The interpolators have trouble with equivalence ratio because its equal
% to 1 over a certain Mach no. (causes the interpolator to find regions of <1 equivalence ratio where they should not exist)

% Anything outside of the region where it is actually changing
% extrapolates to over 1 (which is then set to 1 by RESTM12int)

engine_data_for_eq_interp = [];
j=1;
for i = 1: length(engine_data(:,1))
    if engine_data(i,1) < 5.
        engine_data_for_eq_interp(j,:) = engine_data(i,:);
        j=j+1;
    end
end
...
% scattered.equivalence = scatteredInterpolant(engine_data_for_eq_interp(:,1),engine_data_for_eq_interp(:,2),engine_data_for_eq_interp(:,4), 'linear');
