...
if V(end) > 40000
    if theta(end) < 0
    ThirdStagePayloadMass = gaussmf(theta(end),[0.1 0])*gaussmf(V(end),[10000 40000])*interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),40000, 0, v(end));    
    else
    ThirdStagePayloadMass = gaussmf(V(end),[10000 40000])*interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),40000, theta(end), v(end));    
    end
elseif v(end) < 2500
    if theta(end) < 0
    ThirdStagePayloadMass = gaussmf(theta(end),[0.1 0])*gaussmf(v(end),[1000 2000])*interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),V(end), theta(end), 2000);  
    else
    ThirdStagePayloadMass = gaussmf(v(end),[1000 2000])*interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),V(end), theta(end), 2000);
    end
elseif V(end) < 34000
    if theta(end) < 0
    ThirdStagePayloadMass = gaussmf(theta(end),[0.1 0])*interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),V(end), theta(end), v(end));  
    else
    ThirdStagePayloadMass = interpn(phi_list.course,zeta_list.course,alt_list.course,gamma_list.course,v_list.course,payload_array.course,phi(end),zeta(end),V(end), theta(end), v(end));
    end
...
elseif V(end) >=34000 && V(end) <= 36500
    if theta(end) < 0
    ThirdStagePayloadMass = gaussmf(theta(end),[0.1 0])*interp3(alt_list.fine,gamma_list.fine,v_list.fine,payload_array.fine,phi(end),zeta(end),V(end), theta(end), v(end),'spline');  
    else
    ThirdStagePayloadMass = interpn(phi_list.fine,zeta_list.fine,alt_list.fine,gamma_list.fine,v_list.fine,payload_array.fine,phi(end),zeta(end),V(end), theta(end), v(end),'spline');
    end
end
