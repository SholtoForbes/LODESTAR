...
% 
M1_Grid = reshape(communicator(:,12),[length(unique(communicator(:,1))),length(unique(communicator(:,2)))]);
pres_Grid = reshape(communicator(:,13),[length(unique(communicator(:,1))),length(unique(communicator(:,2)))]);
temp_Grid = reshape(communicator(:,14),[length(unique(communicator(:,1))),length(unique(communicator(:,2)))]);

scattered.M1gridded = griddedInterpolant(MList,AOAList,M1_Grid,'linear','linear');
scattered.presgridded = griddedInterpolant(MList,AOAList,pres_Grid,'linear','linear');
scattered.tempgridded = griddedInterpolant(MList,AOAList,temp_Grid,'linear','linear');
...
[scattered.AoA,scattered.flapdeflection,scattered.drag,scattered.flap_pm] = LiftForceInterp(communicator,communicator_trim,const,Atmosphere, scattered, gridded);
