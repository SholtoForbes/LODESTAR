...
if plotflag == 1
    ...
    plot(t,[rad2deg(Alpha) rad2deg(Alpha(end))], 'LineStyle', '-.','Color','k', 'lineWidth', 1.0)
    ylabel('Altitude (km), Dynamic Pressure (kPa), Angle of Attack (deg)');
    ylim([0 140])
    

    addaxis(t,v, [0 6000], 'LineStyle', '--','Color','k', 'lineWidth', 2.0)
    addaxisplot(t, m,2, 'LineStyle', ':','Color','k', 'lineWidth', 1.4)
    addaxislabel(2,'Velocity (m/s), Mass (kg)');


    addaxis(t,[rad2deg(Vec_angle) rad2deg(Vec_angle(end))], 'LineStyle', ':','Color','k', 'lineWidth', 2.0)
    addaxisplot(t, rad2deg(gamma),3, 'LineStyle', '-','Color','k', 'lineWidth', .5)
