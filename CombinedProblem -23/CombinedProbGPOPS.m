...
% Primal Bounds
bounds.phase(1).state.lower = [Stage2.Bounds.Alt(1), Stage2.Bounds.v(1), Stage2.Bounds.gamma(1), Stage2.Bounds.mFuel(1), Stage2.Bounds.gammadot(1), Stage2.Bounds.zeta(1)];
bounds.phase(1).state.upper = [Stage2.Bounds.Alt(2), Stage2.Bounds.v(2), Stage2.Bounds.gamma(2), Stage2.Bounds.mFuel(2), Stage2.Bounds.gammadot(2), Stage2.Bounds.zeta(2)];

% Initial States
bounds.phase(1).initialstate.lower = [Stage2.Bounds.Alt(1), Stage2.Initial.v, Stage2.Bounds.gamma(1), Stage2.Initial.mFuel, Stage2.Bounds.gammadot(1), Stage2.Bounds.zeta(1)] ;
bounds.phase(1).initialstate.upper = [Stage2.Bounds.Alt(2), Stage2.Initial.v, Stage2.Bounds.gamma(2), Stage2.Initial.mFuel, Stage2.Bounds.gammadot(2), Stage2.Bounds.zeta(2)];

% End States
bounds.phase(1).finalstate.lower = [Stage2.Bounds.Alt(1), Stage2.Bounds.v(1), Stage2.End.gammaOpt(1), Stage2.End.mFuel, Stage2.Bounds.gammadot(1), Stage2.End.Zeta];
bounds.phase(1).finalstate.upper = [Stage2.Bounds.Alt(2), Stage2.Bounds.v(2), Stage2.End.gammaOpt(2), Stage2.End.mFuel, Stage2.Bounds.gammadot(2), Stage2.End.Zeta];

% Control Bounds
bounds.phase(1).control.lower = Stage2.Bounds.control(1);
bounds.phase(1).control.upper = Stage2.Bounds.control(2); 
...
if const == 1 || const == 14 || const == 15
    ...
    bounds.phase(1).path.upper = [50000 ,9];
elseif const == 12
    bounds.phase(1).path.lower = [0 ,0];
...
guess.phase(1).state(:,5)   = Stage2.Guess.gammadot.';
guess.phase(1).state(:,6)   = Stage2.Guess.zeta.';
guess.phase(1).control      = Stage2.Guess.control.';
guess.phase(1).time          = Stage2.Guess.time.';

% Tire stages together
bounds.eventgroup(1).lower = [0, 0, 0, 0];
...
bounds.phase(2).initialtime.lower = t0;
bounds.phase(2).initialtime.upper = tfMax;
bounds.phase(2).finaltime.lower = tfMin;
bounds.phase(2).finaltime.upper = tfMax;

bounds.phase(2).initialstate.lower = [altMin, speedMin, fpaMin, auxdata.ThirdStagem, aoaMin];
bounds.phase(2).initialstate.upper = [altMax, speedMax, fpaMax, auxdata.ThirdStagem, aoaMax];

bounds.phase(2).state.lower = [altMin,speedMin, fpaMin, 0, aoaMin];
bounds.phase(2).state.upper = [altMax, speedMax, fpaMax, auxdata.ThirdStagem, aoaMax];

bounds.phase(2).finalstate.lower = [altMin, speedMin, 0, 0, 0];
bounds.phase(2).finalstate.upper = [altMax, speedMax, fpaMax, auxdata.ThirdStagem, 0];

bounds.phase(2).control.lower = [aoadotMin];
bounds.phase(2).control.upper = [aoadotMax];

bounds.phase(2).path.lower = [-deg2rad(8), -inf];
bounds.phase(2).path.upper = [deg2rad(8), 0];

bounds.eventgroup(2).lower = 100000;
bounds.eventgroup(2).upper = 566000;
...
guess.phase(2).state   = [altGuess, speedGuess, fpaGuess, mGuess, aoaGuess];
...
V = output.result.solution.phase(1).state(:,1).';
v = output.result.solution.phase(1).state(:,2).'; 
gamma = output.result.solution.phase(1).state(:,3).'; 
mfuel = output.result.solution.phase(1).state(:,4).'; 
gammadot = output.result.solution.phase(1).state(:,5).';
